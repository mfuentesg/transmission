...
func DoRequest(request *Request) (Response, error) {
	var empty Response
	...
	req, err := http.NewRequest(http.MethodPost, request.URL, bytes.NewBuffer(body))
	if err != nil {
		return empty, err
	}

	if request.Auth.Password != "" && request.Auth.Username != "" {
		req.SetBasicAuth(request.Auth.Username, request.Auth.Password)
	}

	req.Header.Set("User-Agent", "transmission")
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set(SessionIdHeader, request.sessionId)
	...
	if resp.StatusCode == http.StatusConflict {
		request.sessionId = resp.Header.Get(SessionIdHeader)
